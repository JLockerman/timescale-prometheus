...
func (m *TimeSeries) Unmarshal(dAtA []byte) error {
	...
	if cap(m.Labels) < numLabels {
		newLabels := make([]Label, len(m.Labels), numLabels)
		copy(newLabels, m.Labels)
		m.Labels = newLabels
	}
	if cap(m.Samples) < numSamples {
		newSamples := make([]Sample, len(m.Samples), numSamples)
		copy(newSamples, m.Samples)
